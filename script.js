...
function addBeneficiary() {
  ...
  const beneficiary = {
    ...
    registrationType: document.getElementById("beneficiaryRegistrationType")
      .value,
    registrationNumber: completeNum(
      parseInt(document.getElementById("beneficiaryRegistrationNumber").value),
      14
    ),
    bank: completeNum(
      parseInt(document.getElementById("beneficiaryBank").value),
      3
    ),
    agency: completeNum(
      parseInt(document.getElementById("beneficiaryAgency").value),
      5
    ),
    agencyDv: completeNum(
      parseInt(document.getElementById("beneficiaryAgencyDv").value),
      1
    ),
    account: completeNum(
      parseInt(document.getElementById("beneficiaryAccount").value),
      12
    ),
    accountDv: completeNum(
      parseInt(document.getElementById("beneficiaryAccountDv").value),
      1
    ),
    paymentDate: document
      ...
      .join(""),
    value: completeNum(
      parseInt(document.getElementById("beneficiaryValue").value * 100),
      15
    ),
...
function generateCnab() {
  ...
  const bankCode = completeNum(
    parseInt(document.getElementById("header-bankCode").value),
    3
  );
  const batch = "0000";
  const registrationNumber = completeNum(
    parseInt(document.getElementById("header-registrationNumber").value),
    14
  );
  ...
  const agency = completeNum(
    parseInt(document.getElementById("header-agency").value),
    5
  );
  const agencyDv = completeNum(
    parseInt(document.getElementById("header-agencyDv").value),
    1
  );
  const account = completeNum(
    parseInt(document.getElementById("header-account").value),
    12
  );
  const accountDv = completeNum(
    parseInt(document.getElementById("header-accountDv").value),
    1
  );
  ...
  const numero = completeNum(
    parseInt(document.getElementById("header-numero").value),
    5
  );
  ...
  const headerArquivo = `${bankCode}${batch}${
    RegisterType.HEADER_ARQUIVO
  }${fillWithSpaces(9)}${
    TipoInscricao.CNPJ
  }${registrationNumber}${convenioCode}${agency}${agencyDv}${account}${accountDv}${fillWithSpaces(
    1
  )}${companyName}${bankName}${fillWithSpaces(
    10
  )}${shippingCode}${generationDate}${generationTime}${sequentialNumber}${layoutVersion}${density}${fillWithSpaces(
    69
  )}`;
  const headerLote = `${bankCode}${serviceBatch}${
    RegisterType.HEADER_LOTE
  }${movementType}${serviceType}${serviceForm}${layoutBatchVersion}${fillWithSpaces(
    1
  )}${
    TipoInscricao.CNPJ
  }${registrationNumber}${convenioCode}${agency}${agencyDv}${account}${accountDv}${fillWithSpaces(
    1
  )}${companyName}${message}${logradouro}${numero}${complemento}${cidade}${cep}${uf}${fillWithSpaces(
    18
  )}`;
  ...
  const segments = beneficiaries
    .map((beneficiary, index) => {
      const indiceImpar = index * 2 + 1;
      beneficiary.bank = completeNum(parseInt(beneficiary.bank), 3);
      beneficiary.registrationNumber = completeNum(
        parseInt(beneficiary.registrationNumber),
        14
      );
      ...
      beneficiary.paymentDate = beneficiary.paymentDate
        .split("-")
        .reverse()
        .join("");
      ...
      //credito em conta
      let segmentA = `${bankCode}${serviceBatch}${
        RegisterType.DETALHE
      }${completeNum(parseInt(indiceImpar), 5)}A000000${beneficiary.bank}${
        beneficiary.agency
      }${beneficiary.agencyDv}${beneficiary.account}${beneficiary.accountDv} ${
        beneficiary.name
      }${fillWithSpaces(20)}${beneficiary.paymentDate}BRL${fillWithZeros(15)}${
        beneficiary.value
      }${fillWithSpaces(20)}${fillWithZeros(8)}${fillWithZeros(
        15
      )}${fillWithSpaces(40)}01${fillWithSpaces(10)}0${fillWithSpaces(10)}`; //cpf
      let segmentB = `${bankCode}${serviceBatch}${
        RegisterType.DETALHE
      }${completeNum(parseInt(indiceImpar + 1), 5)}B${fillWithSpaces(3)}${
        beneficiary.registrationType
      }${beneficiary.registrationNumber}${beneficiary.logradouro}${
        beneficiary.numero
      }${beneficiary.complemento}${beneficiary.bairro}${beneficiary.cidade}${
        beneficiary.cep
      }${beneficiary.uf}${generationDate}${beneficiary.value}${fillWithZeros(
        60
      )}${fillWithSpaces(15)}0${fillWithSpaces(14)}`;
  ...
  const trailerLote = `${bankCode}${serviceBatch}${
    RegisterType.TRAILER_LOTE
  }${fillWithSpaces(9)}${registersCount}${totalValue}${fillWithZeros(
    18
  )}${fillWithZeros(6)}${fillWithSpaces(165)}${fillWithSpaces(10)}`;

  registersWithTrailers = completeNum(parseInt(registersWithTrailers), 6);
  const trailerArquivo = `${bankCode}9999${
    RegisterType.TRAILER_ARQUIVO
  }${fillWithSpaces(9)}000001${registersWithTrailers}${fillWithZeros(
    6
  )}${fillWithSpaces(205)}`;
...
function saveBeneficiaryData() {
  ...
  let inputs = document.querySelectorAll(
    "#beneficiaryForm input, #beneficiaryForm select"
  );
  inputs.forEach((input) => {
  ...
  let savedBeneficiaries = JSON.parse(
    localStorage.getItem("savedBeneficiaries") || "[]"
  );
  ...
  localStorage.setItem(
    "savedBeneficiaries",
    JSON.stringify(savedBeneficiaries)
  );
...
function exportToTxt() {
  ...
  let fullCompanyName = document
    .getElementById("header-companyName")
    .value.trim();
